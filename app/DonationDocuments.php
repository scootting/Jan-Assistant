...
class DonationDocuments extends Model
{
    public static function getListDonationDocuments($gestion)
    {        
        $query = "select * from act.ff_desc_donacion('".$gestion."')";
    ...
    public static function getInfoEncargado($nro_doc){
        $query = "select a.nro_doc,a.fecha, a.responsable, a.cargores , a.estado , of.ofc_des
                    ...
                    where a.id_ofc = of.id_oficina and a.tip_doc = 2 and a.nro_doc = '".$nro_doc."'";
        $data = collect(DB::select(DB::raw($query)));
        return $data[0];
    }   
    ...
    {
        $may=0;
        foreach(DonationDocuments::getListDocuments($gestion) as $doc){
