  ...
  <div>
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span>Documentos de Carpinteria</span>
        <el-button
          style="text-align: right; float: right"
                    size="small"
                    type="primary"
                    icon="el-icon-plus"
          @click="newCarpentryDocument"
        >Nuevo Documento de Carpinteria</el-button>
      </div>
...
export default {
  ...
  created() {
    var app = this;
    axios
      .get("/api/carpentryDocuments/" + app.user.gestion)
      .then(response => {
  ...
  methods: {
    ...
    redirectEdit(scope) {
      ...
      this.$router.push({ name: "editcarpentrydocument" });
    },
    generatePDF() {
      console.log("imprimir doc en PDF");
    },
    newCarpentryDocument(scope) {
      console.log(scope);
      this.$store.state.encargado = scope;
      this.$router.push({ name: "addcarpentrydocument" });
    }
  }
};
</script>
